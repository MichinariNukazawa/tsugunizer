...
def expand_rect(rect, crop_info):
	...
	offset_w = rect['w'] * crop_info['width']
	offset_h = rect['h'] * crop_info['top']
	rect['x'] -= int(offset_w / 2)
	rect['w'] += int(offset_w)
	rect['y'] -= int(offset_h / 2)
	rect['h'] += int(rect['h'] * crop_info['bottom'])
...
def crop_image(image, rect, crop_mode):
	...
	return crop_image_(image, rect_)
...
def main():
	...
	parser.add_argument('--width', type=float, default=0.5, help="crop offset of ")
	parser.add_argument('--top', type=float, default=1.0, help="crop offset of ")
	parser.add_argument('--bottom', type=float, default=2.0, help="crop offset of ")
	parser.add_argument('--max_num', type=int, default=-1)
	parsed = parser.parse_args()

	crop_info = {"width": parsed.width, "top": parsed.top, "bottom": parsed.bottom}

	parsed.output_dir = "{}_{}".format(os.path.normpath(parsed.input_dir), "output")
	...
	for i, image_filepath in enumerate(files):
		...
		for t, face_rect in enumerate(face_rects):
			output_imagefile = os.path.join(parsed.output_dir, "{}_f{:02}{}".format(image_filename, t, ".jpg"))
			try:
				#print(image_filepath)
				#pprint.pprint(face_rect)

				image = PIL.Image.open(image_filepath)
				image = crop_image(image, face_rect, parsed.crop_mode)
				if not image:
					continue
				image.save(output_imagefile, quality=95)
				saved_count += 1
			except Exception:
				logging.exception('Failed to image: `%s`', image_filepath)
				continue
